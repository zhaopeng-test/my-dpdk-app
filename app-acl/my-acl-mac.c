...
struct rte_acl_field_def ipv4_defs[7] = {
		...
		},
		{
			...
			.field_index = 5,
			.input_index = 4,
			.offset = offsetof(struct ipv4_5tuple, mac0),
		},
		{
			.type = RTE_ACL_FIELD_TYPE_BITMASK,
			.size = sizeof(uint32_t),
			.field_index = 6,
			.input_index = 5,
			.offset = offsetof(struct ipv4_5tuple, mac1),
		},
...
int main(int argc, void **argv)
{
	...
	struct acl_ipv4_rule acl_rules[] = {
		...
		{
			.data = {.userdata = 2, .category_mask = 1, .priority = 1},
			.field[5] = {.value.u32 = 0x33445566, .mask_range.u32 = 0xffffffff},
			.field[6] = {.value.u32 = 0x00001122, .mask_range.u32 = 0xffffffff},
		},
	};
	...
	uint8_t net_mac_order[6] = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66};
	uint32_t mac0 = 0;
	uint32_t mac1 = 0;
	uint8_t *pm = &mac1;
	...
	//data[0].mac0 = rte_be_to_cpu_32(0x33445566);
	//data[0].mac1 = rte_be_to_cpu_32(0x00001122);
